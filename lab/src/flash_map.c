...
static const struct flash_area FM2[] =
{
 {
     .fa_id        = FLASH_AREA_0_ID,
     .fa_device_id = FLASH_DEVICE_EXTERNAL_FLASH,
...
//@@ Add this function to paste in the appropriate flash map
...
//@@ redefine this  static const struct flash_area flash_map[] =
...
static const uint32_t flash_map_entry_num = sizeof(FM1) / sizeof(struct flash_area);
...
int flash_area_write (const struct flash_area * area, uint32_t off, const void * src, uint32_t len)
{
...
#endif
    {
        ...
        else
        {
            ...
            {
                //@@ ISSUE with the erase and write functions!!!!
                //@@ swap the order, do the operation THEN wait for the get_flash_status!
                //@@ an issue pops up if you try to do a read whilst it's still erasing or writing...
                err =
                    (int) gp_mcuboot_xspi_instance->p_api->write(gp_mcuboot_xspi_instance->p_ctrl,
